...
add_menu_page('Список подписавшихся', 'Список подписавшихся', 'manage_options', 'grafik-rabot.php', 'grafik_rabot','dashicons-phone', 1);}
add_action('admin_menu', 'menu_panel_grafik_rabot');

function grafik_rabot() {
// тут уже будет находиться содержимое страницы
echo "<h2>Список подписавшихся</h2><hr><p>Изменить график работы</p><hr>";
	global $wpdb;
// 	$sql = $wpdb->prepare("SELECT * FROM wp_my_podpiski" );
// 	$my_users =$wpdb->get_results($sql);
// 	echo $my_users;
// 	

	 $results = $wpdb->get_results($wpdb->prepare(
               "SELECT * FROM wp_my_podpiski"), OBJECT );
	echo "<table>";
	echo "<tr>";
	echo "<th> id </th>";
	echo "<th> Имя </th>";
...
foreach ( $results as $result ) {
	echo "<tr>";
	echo "<form action='grafik-rabot.php' method='post'>";
	echo "<td	style='padding:0 15px'>". $result->id."</td>";
	echo "<td style='padding:0 15px'>". $result->name."</td>";
	echo "<td style='padding:0 15px'>". $result->city."</td>";
	echo "<td style='padding:0 15px'>". $result->age."</td>";
	echo "<td style='padding:0 15px'>". $result->tel."</td>";
	echo "<td style='padding:0 15px'>". $result->mail."</td>";
	echo "<td style='padding:0 15px'>". $result->tarif."</td>";
	echo "</form>";
	echo "</tr>";
